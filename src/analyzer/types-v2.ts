// OMS Analysis Pipeline V2 — shared types
...
export interface PatternSummary {
  ...
  coverage_gaps: Array<{
    ...
    keywords: string[];
    covered_by: string[];
    gap_value: string; // "LOW" | "MEDIUM" | "HIGH"
  }>;
  ...
  response_quality: Array<{
    ...
    pct_has_search: number;
  }>;

  prompt_structure_effectiveness: Array<{
    structure: string;
    probes: number;
    avg_intent: number;
    avg_response_length: number;
  ...
  oms_language_penetration: {
    pct_with_oms_language: number;
    top_oms_signals: Array<{ signal: string; count: number }>;
  };
...
export interface OMSAnalysisOutput {
  generated_at: string;
